...
def scrape_savant_data(player_name, game_id):
    ...
    df['3D wOBAcon'] = [sum(np.multiply(xwOBAcon_model.predict_proba([[x,y,z]])[0],np.array([0,0.9,1.25,1.6,2]))) if all([i != None for i in [x,y,z]]) else None for x,y,z in zip(df['Spray Angle'],df['Launch Angle'],df['Launch Speed'])]
