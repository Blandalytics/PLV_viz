...
def stuff_chart(df,player,palette):
    ...
    if palette != 'plvStuff+':
        for pitch in [x for x in chart_df['pitchtype'].loc[].value_counts().index if x in chart_df['pitchtype'].unique()]:#set(chart_df['pitchtype'].value_counts().index) & set(pitch_order):
