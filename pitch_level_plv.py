...
def load_data(year):
    ...
    for month in range(3,11):
        ...
        if pd.read_parquet(file_name).shape[0]==0:
            continue
        df = pd.concat([df,
...
games = list(year_data.loc[year_data['pitchername']==player,'game_played'].sort_values(ascending=False).unique())
game_date = st.selectbox('Choose a game:', games, index=0)
